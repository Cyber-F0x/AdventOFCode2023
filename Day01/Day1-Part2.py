...
def get_ints_from_cal(cal_val):
    ...
    last = None
    # Easy checks first
    if str(cal_val[0]) in ints :
        first = str(cal_val[0])
        
    if str(cal_val[-1]) in ints :
        last = str(cal_val[-1])
    ...
    for each_key in mapping:
        if str(each_key) in cal_val:
            indexes.append((cal_val.index(each_key),str(mapping[each_key])))
            indexes.append((cal_val.rindex(each_key),str(mapping[each_key])))
    filtered_cal = [eval(i) for i in list(filter(filter_ints,cal_val))]    
    for each_int in filtered_cal:
        indexes.append((cal_val.index(str(each_int)),str(each_int)))
        indexes.append((cal_val.rindex(str(each_int)),str(each_int)))
